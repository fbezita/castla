...
## 8. system_server 데드락 방지 동기화 및 무중단 비디오 핫리프레시, 스마트 앱 페어 일반화 개량 (2026-05-20 최신 업데이트)
...
### 1) Android system_server 데드락 차단 (`vdOperationGlobalMutex` 및 지연 가드 도입)
* **상태 동시성 제어**: 
  - Primary 및 Secondary 파이프라인의 가상 디스플레이 생성, 소멸, 해상도 리사이즈 트랜잭션이 비동기적으로 동시에 발생하며 안드로이드 그래픽 드라이버/DisplayManager를 자극해 데드락을 유발하던 문제를 발견했습니다.
  - 이를 방어하고자 `MirrorForegroundService.kt`에 전역 수준의 동기화 뮤텍스 `private val vdOperationGlobalMutex = Mutex()`를 탑재하여 두 디스플레이 조작 과정을 완벽히 직렬화시켰습니다.
* **Secondary Safe Detached Resize 구축**:
  - Secondary 파이프라인 리사이즈 시에도 `setSurface(null)` ➔ `delay(50)` ➔ `resizeDisplay` ➔ `delay(50)` ➔ `setSurface(surface)` 가드를 빈틈없이 이식하여 윈도우 매니저 버퍼 경합을 완벽하게 진압했습니다.

### 2) Zero-Restart 무중단 비디오 핫리프레시 (Hot-Refresh) 및 초기 기동 검은 화면 (Black Screen) 완치
* **소켓 접속 연결 유지**:
  - 화면 비율 드래그 또는 버튼 조작 시 비디오 소켓을 억지로 끊었다가 다시 맺는 낡은 처리 방식을 철폐했습니다.
  - 활성화된 영상 웹소켓 세션은 끊지 않고 그대로 유지한 상태에서, WebCodecs 디코더 객체만 원자적으로 기민하게 재생성(`initDecoder`, `initSecondaryDecoder`)하도록 변경했습니다.
* **즉각적 키프레임 전송 유도**:
  - 디코더 교체 즉시 제어 웹소켓을 통해 백엔드로 키프레임(`requestKeyframe`)을 전송받아 헤더 누락으로 인한 5초 지연 및 검은 화면("Connecting...")을 완전히 박멸하고 부드러운 스케일링을 구현했습니다.
* **[CRITICAL] 디코더 최초 기동 키프레임 대기 해제 정밀화 (`waitingForKeyframe` 선행 해제)**:
  - **장애 원인 규명**: 핫리프레시 또는 최초 화면 로드 시, WebCodecs 디코더 객체가 완전히 새롭게 생성되면 `_lastSeqNum`은 `undefined`로 초기화됩니다. 이 상태에서 가장 첫 번째로 도착한 키프레임(flags=0x01)을 수신하면, 기존의 시퀀스 갭 검증 블록(`if (this._lastSeqNum !== undefined)`)이 통째로 건너뛰어지는 바람에 `_waitingForKeyframe = false` 상태 잠금이 해제되지 못하고 평생 `true` 상태로 고착되는 미시적 결함이 있었습니다. 이로 인해 최초 키프레임 이후에 도착한 모든 델타 프레임들이 `_waitingForKeyframe` 게이트에 가로막혀 백그라운드에서 전부 영구 드롭되었고, 그 결과 2번째 키프레임이 들어올 때까지 화면이 **영구적인 검은 화면(Black Screen)**에 갇히는 타이밍 락이 발생했습니다.
  - **해결책**: NAL Unit을 디코드하는 최상단에서 키프레임(isKeyFrame = true) 수신 즉시, 시퀀스 번호의 정합성 유무 및 `_lastSeqNum` 정의 여부에 무관하게 **최우선으로 `this._waitingForKeyframe = false`를 해제하고 시퀀스 트래킹을 즉각 동기화(Re-anchoring)하는 최선행 잠금 해제 파이프라인**을 구축했습니다. 이를 통해 핫리프레시 직후 단 1프레임의 델타 버퍼 손실도 없이 완벽하고 매끄러운 60fps 무중단 미러링 전환을 완성했습니다.
* **[CRITICAL] Split-to-Single 전환 시 Primary Canvas 투명도 소실 버그 완치**:
  - **장애 원인 규명**: 듀얼 스플릿 상태에서 `splitExpandLeftBtn`을 누르면, Left에 실행되고 있던 Primary 앱이 화면 전체로 극대화됩니다. 이때 프론트엔드는 비디오 소켓 세션을 재생성하는 비효율적인 재연결 방식 대신 **무중단 핫리프레시(Zero-Restart Hot-Refresh)** 모드로 동작하여 뷰포트 해상도 크기만 리사이즈 조절하게 됩니다. 그러나 프론트엔드 레이아웃 엔진인 `updateLayoutUI()`의 `hasLeft` 분기문 내부에서 과거 단일 스트림 시절의 유물인 `clearCanvas()`가 습관적으로 호출되어 **`canvas.style.opacity = '0'`**으로 강제 은닉시키는 버그가 존재했습니다. 무중단 핫리프레시이므로 웹소켓이 끊기지 않아 최초 프레임 수신 핸들러(`checkReady`)가 재작동하지 못했고, 그 결과 프라이머리 캔버스가 백그라운드에서는 60fps로 실시간 프레임을 성실히 그리면서도 사용자 브라우저상에는 **검은 투명 스크린(Black Screen)**으로 은폐되는 크리티컬 락이 유발되었습니다.
  - **해결책**: `updateLayoutUI()`의 `hasLeft` 분기문에서 `clearCanvas()` 호출을 철폐하고, 무중단 리사이즈 중에도 프라이머리 비디오 스트림이 1밀리초의 끊김도 없이 연속 노출되도록 **`canvas.style.opacity = '1'`** 가드 정책을 명시적으로 확립했습니다. 이를 통해 Left 최대화 클릭 시 검은 깜빡임조차 전혀 없는 명품 심리스(Seamless) 확대 연출을 완성시켰습니다.
* **[CRITICAL] 무중단 해상도 유지 기동 시 로딩 오버레이 타임아웃 ("Launch timed out") 결함 해결**:
  - **장애 원인 규명**: 브라우저 스플릿 레이아웃 변경 또는 기동 중인 상태에서 사이드바의 단일 앱 아이콘을 클릭하여 새로운 앱을 구동할 때, 브라우저 가상 화면의 해상도가 기존과 동일하다면 백엔드 가상 디스플레이는 리빌드(Rebuild) 및 리사이즈(Resize)를 거치지 않습니다. 이 때문에 인코더는 SPS/PPS 및 새로운 키프레임(Keyframe)을 자발적으로 발행하지 않고 기존의 델타 프레임(Delta Frame)들만 계속 전송하게 됩니다. 그러나 프론트엔드의 `launchApp()`에서는 신규 구동 시 무조건 `firstFrameReceived = false`로 설정하고 "Loading..." 오버레이를 띄운 뒤 H264 키프레임 수신(`v[0] === 0x01`)만을 목매어 기다렸습니다. 델타 프레임들은 계속 도착하여 캔버스를 성실히 그리고 있었으나, 오버레이 상태 코드가 해제되지 못하여 결국 **5초 타임아웃 경고("Launch timed out. Try again.")**가 화면에 표시되는 사용자 경험 훼손이 발생했습니다.
  - **해결책**: 
    ...
    3. **[락 뷰포트 정합성 보존]**: 스플릿 해제 분기(`hasLeft`) 진입 시 `leftLockedViewport` 및 `rightLockedViewport`를 명시적으로 `null` 초기화하여, 단일 풀 스크린 상태에서 디버깅 로그상 쪼개진 화면 사이즈가 잔존하여 뷰포트 핑퐁을 부르는 미세 흔들림 가능성까지 사전에 완전히 청소했습니다.

### 3) 지능형 스마트 앱 페어 교차 감지 (Smart Layout Matching Engine)
* **최적 경로 일반화 수식 적용**:
  - 앱 페어 `(X, Y)` 요청 시 화면에 켜져 있는 기존 앱 `(A, B)`의 위치와 패키지명을 대조하는 일반화 알고리즘을 이식했습니다.
  - **결과 규칙**:
    - **Case 1 (완전 일치):** `X`, `Y`가 이미 화면에 다 올라와 있다면 실행을 통째로 생략하여 중복 구동을 방지합니다. (`실행 안함`)
  ...
  - **단일 화면 상태 보정**: `A`만 켜져 있을 때 `(A, B)` 페어를 클릭하면, `A`를 그대로 유지하면서 `B`만 Secondary 칸에 단독 런칭하여 화면 분할과 사이즈 조절이 물 흐르듯 실시간 연동됩니다.

---

## 9. 미러링 가상 디스플레이 블랙 화면 및 백엔드 셧다운 데드락 장애 해결 (2026-05-20 최신 업데이트)

* **배경 및 원인**:
  - 미러링 중지 클릭 시 내부 `release()`가 비동기 코루틴 상에서 실행되는 도중 바인더 세션이 먼저 unbind되거나 `serviceScope.cancel()`이 트리거되어, 하드웨어 코덱 Surface와 가상 디스플레이 커널 리소스가 제대로 반납되지 못한 채 방치되었습니다. 이로 인해 드라이버 단의 뮤텍스(Mutex) 락을 쥔 채 시스템 전체가 얼어붙어 스마트폰 전체가 리부팅되는 치명적인 교착이 유발되었습니다.
  - 최초 구동 시 뷰포트 해상도 변경 락(`pipelineMutex`)이 가상 화면 생성이 완료되기 전에 조기 해제되어, 뒤이은 두 번째 리빌드 작업과 꼬이면서 인코더가 유실되는 경합 및 `setSurface(null)` 호출로 인한 삼성 Exynos 기기의 가상 디스플레이 즉시 슬립(STATE_OFF) 결함이 존재했습니다.
* **해결 및 개선 조치**:
