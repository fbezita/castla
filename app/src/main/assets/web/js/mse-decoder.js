...
class MseDecoder {
  constructor(videoElementId = "mse-video", onError) {
    ...
    this.maxLatency = 0.4; // 400ms maximum tolerable latency
  ...
  init() {
    return new Promise((resolve, reject) => {
      if (!this.video) {
        console.error(
          `[MSE] Video element #${this.videoElementId} not found in DOM`,
        );
      ...
      try {
        ...
        const canvasId =
          this.videoElementId === "mse-video-secondary"
            ? "display-secondary"
            : "display";
        ...
        this.ready = true;
        console.log(
          `[MSE] Pre-initialization finished for #${this.videoElementId}. Awaiting H.264 stream to start JMuxer.`,
        );
        resolve();
        /* ### 수정 끝 ### */
      } catch (e) {
  ...
  ensureJmuxerInitialized(codecStr = null) {
    ...
    console.log(
      `[MSE] Initializing JMuxer lazily on #${this.videoElementId}. Detected H.264 Codec: ${codecStr || "avc1.64001f"}`,
    );
  ...
  decode(data) {
    ...
    if (flags === 0x02) {
      ...
      console.log(
        `[MSE] Intercepted H.264 SPS/PPS config packet. Size: ${this._cachedSpsPps.byteLength}`,
      );
      return;
    }
    ...
    if (!this.jmuxer) {
      const codecStr = this._cachedSpsPps
        ? this.detectCodecFromSps(new Uint8Array(this._cachedSpsPps))
        : null;
  ...
  setFitMode(fitMode) {
    ...
    console.log(
      `[MSE] Dynamic fitMode updated on #${this.videoElementId}: ${fitMode}`,
    );
  }
