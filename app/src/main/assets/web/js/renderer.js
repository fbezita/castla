...
class CanvasRenderer {
  ...
  render(frame) {
    ...
    if (sourceWidth !== this.videoWidth || sourceHeight !== this.videoHeight) {
      ...
      if (this.tempFillPending) {
        ...
        const canvasHeight =
          this.canvas.clientHeight || window.innerHeight || 1;
        ...
        if (
          this.tempFillTargetAspect &&
          Number.isFinite(this.tempFillTargetAspect) &&
          this.tempFillTargetAspect > 0
        ) {
        ...
        if (this.tempFillTargetMode === "single") {
          ...
          if (aspect < canvasAspect - 0.08) {
            console.log(
              `[Renderer] Aspect guard: received frame aspect ${aspect.toFixed(2)} while expecting fullscreen aspect >= ${(canvasAspect - 0.08).toFixed(2)} (${sourceWidth}x${sourceHeight}), holding fill mode.`,
            );
            shouldRelease = false;
          }
          ...
          if (aspect > canvasAspect + 0.08) {
            console.log(
              `[Renderer] Aspect guard: received frame aspect ${aspect.toFixed(2)} while expecting split aspect <= ${(canvasAspect + 0.08).toFixed(2)} (${sourceWidth}x${sourceHeight}), holding fill mode.`,
            );
            shouldRelease = false;
          }
        }
        ...
        if (shouldRelease) {
          console.log(
            `[Renderer] Settled aspect/resolution ${sourceWidth}x${sourceHeight} (aspect: ${aspect.toFixed(2)}) received matching target aspect, clearing tempFillPending.`,
          );
          this.tempFillPending = false;
          this.tempFillTargetMode = null;
          this.tempFillTargetAspect = null;
  ...
  canvasToVideo(canvasX, canvasY) {
    ...
    if (
      !this.renderWidth ||
      isNaN(this.renderWidth) ||
      this.renderWidth <= 0 ||
      !this.renderHeight ||
      isNaN(this.renderHeight) ||
      this.renderHeight <= 0
    ) {
